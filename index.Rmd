...
includeScript("www/syncCharts.js")
...
rv$stateAgeGroupCases <- "<h5 class='chart-title-text'>Total Cases by Age Group</h5>"
  ...
  input$stateAgeGroupChartType, {
    switch(
      ...
      "Cumulative Totals" = 
        rv$stateAgeGroupCases <- "<h5 class='chart-title-text'>Total Cases by Age Group</h5>",
      "Daily New Values" = 
        rv$stateAgeGroupCases <- "<h5 class='chart-title-text'>New Cases by Age Group</h5>"
    )
  }
)

casesAgeGroup <- eventReactive(
  input$stateAgeGroupChartType, {
    output <- switch(
      input$stateAgeGroupChartType,
      "Cumulative Totals" = covid$SAGCMSBD %>%
        filter(AgeGroup != "Missing") %>%
        select(
          "plotX" = AgeGroup,
          "plotY" = TotalCases
        ),
      "Daily New Values" = covid$SAGCNMS %>%
        filter(AgeGroup != "Missing") %>%
        select(
          "plotX" = AgeGroup,
          "plotY" = NewCases
...
stateAgeGroupCasesYAxisTitle <- eventReactive(
  input$stateAgeGroupChartType, {
    output <- switch(
      input$stateAgeGroupChartType,
...
renderHighchart(
  HighChartBarDynamic(
    .data = casesAgeGroup,
    .yTitle = stateAgeGroupCasesYAxisTitle
  ) %>%
    StyleBarDynamic()
)
...
rv$stateAgeGroupHospitalizations <- "<h5 class='chart-title-text'>Total Hospitalizations by Age Group</h5>"
  ...
  input$stateAgeGroupChartType, {
    switch(
      ...
      "Cumulative Totals" = 
        rv$stateAgeGroupHospitalizations <- "<h5 class='chart-title-text'>Total Hospitalizations by Age Group</h5>",
      "Daily New Values" = 
        rv$stateAgeGroupHospitalizations <- "<h5 class='chart-title-text'>New Hospitalizations by Age Group</h5>"
    )
  }
)

hospitalizationsAgeGroup <- eventReactive(
  input$stateAgeGroupChartType, {
    output <- switch(
      input$stateAgeGroupChartType,
      "Cumulative Totals" = covid$SAGCMSBD %>%
...
stateAgeGroupHospitalizationsYAxisTitle <- eventReactive(
  input$stateAgeGroupChartType, {
    output <- switch(
      input$stateAgeGroupChartType,
...
renderHighchart(
  HighChartBarDynamic(
    .data = hospitalizationsAgeGroup,
    .yTitle = stateAgeGroupHospitalizationsYAxisTitle
  ) %>%
    StyleBarDynamic()
)
...
stateAgeGroupDeathsYAxisTitle <- eventReactive(
  input$stateAgeGroupChartType, {
    output <- switch(
      input$stateAgeGroupChartType,
...
renderHighchart(
  HighChartBarDynamic(
    .data = deathsAgeGroup,
    .yTitle = stateAgeGroupDeathsYAxisTitle
  ) %>%
    StyleBarDynamic()
)
...
### Chart
```{r}
stateAgeGroupChartData <- eventReactive(
  c(input$stateAgeGroupMetric, input$stateAgeGroupChartType, input$ageGroups), {
    chosenMetric <- switch(
      input$stateAgeGroupMetric,
      "Total Cases" = "TotalCases",
      "Total Hospitalizations" = "Hospitalizations",
      "Total Deaths" = "Deaths",
      "Cases per 100K" = "CasesPer100K",
...
stateAgeGroupYAxisTitle <- eventReactive(
  input$stateAgeGroupMetric, {
    output <- input$stateAgeGroupMetric
  }
)

stateAgeGroupSummaryPlotter <- eventReactive(
  c(input$stateAgeGroupMetric, input$ageGroups), {
    switch(
      input$stateAgeGroupChartType,
      "Cumulative Totals" = HighChartMultiLineDate(
        stateAgeGroupChartData, 
        stateAgeGroupYAxisTitle
      ) %>%
        StyleMultiLineDate(
          stateAgeGroupYAxisTitle, 
          chartSubtitle
...
Health Planning Region Metric Summary {data-navmenu="Health Planning Regions"}
================================================================================
...
Locality Metric Summary {data-navmenu="Localities"}
===============================================================================
...
tags$script("
    Shiny.addCustomMessageHandler('reset', function(input) {
      Shiny.onInputChange(input, null);
    });"
)

observeEvent(input$mapReset, {
    session$sendCustomMessage(type = "reset", message = "mapClick")
  })



# reactive object to store the name of the chosen locality
...
# reactive value to store the total summary chart title
rv$localityTotalSummaryTitle <- 
  "<h5 class='chart-title-text'>Totals By Date for Virginia</h5>"
  
# change chart title based on the chosen locality
# FIRES ON MAP CLICK AND MAP RESET
observeEvent(
  c(input$mapClick, input$mapReset), {
...
# reactive object for chart data
# FIRES ON METRIC VALUE CHANGE AND MAP CLICK
localitySummaryTotalChartData <- eventReactive(
  c(input$localitySummaryMetric, input$mapClick), {
    chosenMetric <- switch(
...
# reactive object for the y-axis title
# FIRES ON METRIC VALUE CHANGE
summaryTotalYAxisTitle <- eventReactive(
  input$localitySummaryMetric, {
...
renderHighchart(
  HighChartSingleLineDate(
    localitySummaryTotalChartData, 
    summaryTotalYAxisTitle
  ) %>%
    StyleSingleLineDate(
      summaryTotalYAxisTitle, 
      chartSubtitle
...
# reactive value to store the new summary chart title
rv$localityNewSummaryTitle <- 
  "<h5 class='chart-title-text'>New By Date for Virginia</h5>"
...
# reactive object for chart data
# FIRES ON METRIC VALUE CHANGE AND MAP CLICK
localitySummaryNewChartData <- eventReactive(
  c(input$localitySummaryMetric, input$mapClick), {
    chosenMetric <- switch(
      input$localitySummaryMetric,
...
# reactive object for the y-axis title
# FIRES ON METRIC VALUE CHANGE
summaryNewYAxisTitle <- eventReactive(
  input$localitySummaryMetric, {
    output <- switch(
      input$localitySummaryMetric,
...
renderHighchart(
  HighChartAreaLineDate(
    localitySummaryNewChartData, 
    summaryNewYAxisTitle
  ) %>%
    StyleAreaLineDate(
      summaryNewYAxisTitle, 
      chartSubtitle
...
# reactive object for chart data
# FIRES ON METRIC VALUE CHANGE
localitySummaryMapData <- eventReactive(
  input$localitySummaryMetric, {
    chosenMetric <- switch(
      input$localitySummaryMetric,
      "Total Cases" = "TotalCases",
    ...
    output <- covid$LCMS %>%
      select(
        fips, 
        Locality, 
        "plotValue" = all_of(chosenMetric)
      )
  }
)

# reactive object for the map title
# FIRES ON METRIC VALUE CHANGE
summaryMapTitle <- eventReactive(
  input$localitySummaryMetric, {
    output <- input$localitySummaryMetric
  }
)
...
# render the chart
renderHighchart(
  HighChartVAChloroplethMap(
    localitySummaryMapData, 
    "plotValue", 
    summaryMapTitle
  )
)
...
### `r renderUI(HTML(rv$localityQuickSummaryTitle))`
```{r hm_locality_quick_summary}

rv$localityQuickSummaryTitle <- 
  "<h5 class='chart-title-text'>Quick Summary for Virginia</h5>"
...
observeEvent(
  c(input$mapClick, input$mapReset), {
    ...
    rv$localityQuickSummaryTitle <- paste0(
      "<h5 class='chart-title-text'>Quick Summary for ",
       chosenLocality(),
      "</h5>"
    )
  }
...
quickSummaryData <- reactive({
  if (is.null(input$mapClick)) {
    output <- bind_cols(
    covid$SCMS, 
    covid$SCNMS[, 2:4]
  ) %>%
    mutate(
      CasesIncrease = NewCases / (TotalCases - NewCases),
      PositivityRate = TotalCases / Population2019,
      HospitalizationsIncrease = 
        NewHospitalizations / (Hospitalizations - NewHospitalizations),
      DeathsIncrease = NewDeaths / (Deaths - NewDeaths)
    ) %>%
    select(
      "Population" = Population2019,
      "Total Cases" = TotalCases,
      "New Cases" = NewCases,
      "Cases Increase" = CasesIncrease,
      "Cases per 100K" = CasesPer100K,
      "Case Rate" = PositivityRate,
      "Total Hospitalizations" = Hospitalizations,
      "New Hospitalizations" = NewHospitalizations,
      "Hospitalizations Increase" = HospitalizationsIncrease,
      "Hospitalizations per 100K" = HospitalizationsPer100K,
      "Hospitalization Rate" = PercentCasesHospitalized,
      "Total Deaths" = Deaths,
      "New Deaths" = NewDeaths,
      "Deaths Increase" = DeathsIncrease,
      "Deaths per 100K" = DeathsPer100K,
      "Mortality Rate" = MortalityRate
    ) %>%
    mutate(
      across(
        .cols = contains(c("Rate", "Increase")),
